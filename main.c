...
int main(void){
	initialize ();						
	/*float R = cos (M_PI/4);
	material *left = add_mat (DEF, req (0,0,1));
	material *right = add_mat (DEF, req (1,0,0));
	add_obj (req(-R, 0, -1), R, left);
	add_obj (req( R, 0, -1), R, right);
*/
	...
	add_obj (req(0.0, -100.5, -1.0), 100, ground);		
